...
class UnifiedSteganographyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply the specified transform only if it is not None
        if self.transform:
...
# Remove the RandomResizedCrop and RandomHorizontalFlip from the transform
...
class CustomSteganalysisModel(nn.Module):
    def __init__(self, num_classes=len(all_file_lists), dropout_rate=0.5):
...
optimizer = optim.Adam(steganalysis_model.parameters(), lr=0.001)  # Adjusted learning rate
...
for epoch in range(100):
    ...
    print(
        f'\nEpoch {epoch + 1}/{5} - Loss: {loss.item():.4f} - Validation Accuracy: {val_accuracy * 100:.2f}% - Epoch Time: {epoch_duration:.2f} seconds')
